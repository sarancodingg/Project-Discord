...
class topupView(nextcord.ui.View):
    ...
    @nextcord.ui.button(label="เช็คสต็อกคีย์", style=nextcord.ButtonStyle.green)
    async def prompt_stock_check(self, button: Button, interaction: nextcord.Interaction):
        # สร้าง View ใหม่ที่มี Dropdown เพื่อให้ผู้ใช้เลือกไฟล์สต็อก
        select_view = View()
        select_view.add_item(StockCheckSelect())
        await interaction.response.send_message("กรุณาเลือกไฟล์สต็อกที่ต้องการเช็ค:", view=select_view, ephemeral=True)


    @nextcord.ui.button(label="เช็ค Top Donator", style=nextcord.ButtonStyle.green, custom_id="check_top_donator")

    async def check_top_donator(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        # โหลดข้อมูลจาก user.json
        with open('./database/users.json', 'r', encoding='utf-8') as f:
            users = json.load(f)
        
        # คำนวณหาผู้ใช้ที่มี all-point สูงสุด
        top_donator = max(users.items(), key=lambda x: x[1]['all-point'])
        user_id, details = top_donator

        # ส่งข้อความกลับไปยังช่องที่มีปุ่มถูกกด
        await interaction.response.send_message(
            f"ผู้ใช้ที่เติมเงินมากที่สุดคือ: <@{user_id}> ด้วยจำนวนเงินรวม {details['all-point']} points",
            ephemeral=True
        )
...
async def checkstock(ctx):
    view = StockCheckView()
    await ctx.send("Click 'Check Stock' to select which stock file to check:", view=view)
